...
class Compsite_display:
    ...
    def binary_frame_mask(self,table=None):
        '''
        ...
        if table is None:
            info_table = pd.DataFrame(
                measure.regionprops_table(
                    self.mask_roi,
                    intensity_image=self.input_image,
                    properties=['area', 'label', 'centroid'],
                )).set_index('label')
        else:
            info_table = table
        info_table['label'] = range(2, len(info_table) + 2)
        framed_mask = np.zeros(np.shape(self.mask_roi), dtype=np.int32)
        if len(info_table['label']) > 1:
            for i in info_table.index.values:
                framed_mask = framed_mask + Compsite_display.outline_seg(mask = self.mask_roi, index = i)
        else:
            framed_mask = Compsite_display.outline_seg(mask = self.mask_roi,index = info_table.index.values)
        return framed_mask
    ...
    def draw_ROI_contour(self,channel=None):
        ...
        if len(np.unique(mask_input)) > 1:
            bin_mask = Compsite_display.binary_frame_mask()
